...
const DailyForecast = () => {
	...
	const { city, time, days } = useWeather()

	useEffect(() => {
		const fetchForecastData = async () => {
			try {
				const result = await axios.get(
					`https://api.openweathermap.org/data/2.5/forecast?q=${city}&lang=fr&cnt=${days}&units=metric&appid=${process.env.REACT_APP_WEATHER_API}`
				)
